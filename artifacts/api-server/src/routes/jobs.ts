...
import { enqueueJob, queuePosition } from "../lib/pipeline-runner";
...
router.post("/jobs", upload.single("file"), async (req, res) => {
  ...
  enqueueJob(job.id, req.file.path, outputDir);
...
router.get("/jobs/:id", async (req, res) => {
  ...
  res.json({
    ...
    status: job.status,
    stage: job.stage,
    queuePosition: job.status === "queued" ? queuePosition(job.id) : 0,
    summary: job.summary,
